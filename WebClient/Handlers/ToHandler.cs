...
namespace TrainNotifier.WebClient.Handlers
{
    public class ToHandler
    {
        ...
        public ToHandler(HttpResponseBase response)
        {
            _response = response;
            _webApiService = new WebApiService(string.Concat("http://", _apiUrl), _apiName);
        }

        public static bool IsToRequest(Uri uri)
        {
            return uri.Query.Replace(HandlerHelper.QueryFragment, string.Empty).Split('/').First().Contains("to");
        }
        ...
        public void ProcessRequest(Uri url)
        {
            ...
            _response.Write(string.Format("<title>Trains to {0} on {1:dd/MM/yyyy} between {2:HH:mm} and {3:HH:mm}</title>",
                station.Description, today, startTime, endTime));
            _response.Write(string.Format("<meta name=\"description\" content=\"Trains to {0} on {1:dd/MM/yyyy} between {2:HH:mm} and {3:HH:mm}\">",
                station.Description, today, startTime, endTime));

            _response.Write("</head><body>");

            _response.Write(string.Format("<h1>Trains to {0}</h1>", station.Description));
            _response.Write(string.Format("<h2>On {0:dd/MM/yyyy} between {1:HH:mm} and {2:HH:mm}</h2>", today, startTime, endTime));
            _response.Write("<hr />");

            TrainMovementResults results = _webApiService.TerminatingAtStation(atCrsCode, startTime, endTime).Result;
